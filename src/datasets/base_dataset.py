...
from src.utils.procrustes_align import procrustes_align
...
class BaseFaceMeshDataset(Dataset, ABC):
    ...
    def __getitem__(self, idx):
        ...
        pts = procrustes_align(pts, self.reference_mesh)
        if self.transform:
    ...
    def sample(self, n: int, random_state: int) -> Self:
        ...
        """
        return self._create_new_dataset(self.data_df.sample(n=n, random_state=random_state, replace=True))
            ...
            for key, add_size in additional_probs.items():
                if key in grouped:
                    ...
                    if sample_size > 0 and count > 0:
                        ...
                        sample = filtered_df.sample(n=sample_size, replace=(sample_size > count),
    ...
    def sample_from_label_column(self, n: int, label_value: int, random_state: int) -> Self:
        ...
        if n < 0:
            samples = data_with_label_value.sample(frac=1, random_state=random_state, replace=True)
