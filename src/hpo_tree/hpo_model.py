...
class HumanPhenotypeModel(HumanPhenotypeTerm):
        ...
        if self.parallel and len(kfold_datasets) > 1:
            ...
            with ProcessPoolExecutor(mp_context=torch.multiprocessing.get_context('spawn')) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    np.save(os.path.join(log_fold_path, 'global_feature_importance.npy'), feature_importance)
    ...
    def _train_fold(self, fold, train_dataset: BaseFaceMeshDataset, val_dataset: BaseFaceMeshDataset,
        ...
        checkpoint_files = self.find_files(output_dir)
        ...
        train_loader = DataLoader(train_dataset, batch_size=batch_size_train,
                                  ...
                                  multiprocessing_context=mp_context if persistent_workers else None)
        ...
        if not os.path.exists(global_feature_importance_file):
            ...
            trainer = Trainer(max_epochs=25, logger=tb_logger, callbacks=callbacks, log_every_n_steps=1,
                              accelerator='gpu', default_root_dir=tmp)

            def handle_sigterm(signum, frame):
                trainer.should_stop = True
                logger.error("Received SIGTERM (%s) – will shut down gracefully after current step", signum)
        ...
        # Export to ONNX
        self.export_to_onnx(output_dir, val_dataset)

        feature_importance = investigate_feature_importance(output_dir, lightning_module, val_dataset)
        checkpoint_file = self.find_files(output_dir)[-1]
    ...
    def predict(self, facemeshes: torch.Tensor, use_metric: Literal[
        ...
        fold_model_ckpts = [self.find_files(os.path.join(self.log_path, fold))[-1] for fold in fold_dir]
    ...
    def export_to_onnx(self, export_dir: str, val_dataset: BaseFaceMeshDataset = None, optimize: bool = True):
        """
        ...
        """
        logger.debug(f'Exporting to ONNX...')
        fold_model_ckpt = self.find_files(export_dir)
        point_mask = np.load(self.point_mask_file_input)
        pl_model = FaceMeshLightningModule.load_from_checkpoint(fold_model_ckpt[-1],
                                                                model=create_model(point_mask, self.dimensions,
                                                                                   self.meta_data),
                                                                map_location='cpu')
        data_structure = {meta_item: torch.randint(-1, 0, (1,)).type(torch.float32) for meta_item in self.meta_data}
        input_data = (torch.randn(1, self.dimensions, 478)[:, :, point_mask], data_structure)
        ...
        if optimize:
            logger.debug('Optimizing ONNX file...')
        ...
        result_dict = self.export_result_dict()

        if val_dataset:
            logger.debug('Collecting ONNX logits for calibration...')
            # If validation dataset is not None, run calibration
            collected = collect_onnx_logits(onnx_file, self, val_dataset)
            if collected:
                calibrators = calibrate_all(collected)
                result_dict.update({'calibration': calibrators})

        with open(os.path.join(export_dir, 'onnx_config.json'), mode='w') as f:
            json.dump(result_dict, f, indent=2)
        logger.debug(f'ONNX file exported to {onnx_file}')
    ...
    def find_files(output_dir: str, file_extension: str = 'ckpt', sort_by_last_modified: bool = True) -> List[pathlib.Path]:
        ...
        Args:
            ...
            sort_by_last_modified (bool, optional): Whether to sort the files by last modified time. Defaults to True.

        Returns:
            List[pathlib.Path]: List of paths to checkpoint files.
        """
        if not os.path.exists(output_dir):
            return []
        ckpt_files = list(pathlib.Path(output_dir).rglob(f'*.{file_extension}'))
        if sort_by_last_modified:
            ckpt_files = sorted(ckpt_files, key=os.path.getmtime, reverse=False)
        return ckpt_files
    ...
    def export_results_json(model: "HumanPhenotypeModel", reference_mesh: np.ndarray, output_path: str = None,
        ...
        Args:
            ...
            output_path (str): Output directory for the result json file.
        """
        root = model.find_root()
        output_dir = output_path if output_path else root.log_path
        os.makedirs(output_dir, exist_ok=True)
        export_path = os.path.join(output_dir, 'result.json')
        data = {
            "reference_mesh": [{
                                   "id": idx,
