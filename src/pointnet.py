...
class RobustNorm(nn.Module):
    ...
    def forward(self, x):
        """
        Forward pass of RobustNorm.
        :param x: Input tensor. For num_groups=1, expected shape [batch, features].
                  Otherwise, expected shape [batch, features, points, 1] or similar.
        :return: Normalized tensor.
        """
        if x.size(0) > 1:
            return self.bn(x)
        else:
            # GroupNorm can fail if training=True and each group has only 1 value
            # This happens if num_features == num_groups and spatial dims are 1x1
            # We check if numel per group is 1.
            num_groups = self.gn.num_groups
            if self.training and (x.numel() // num_groups) <= 1:
                return x  # Skip normalization if only 1 value per group
            return self.gn(x)
...
class BasePointNet(nn.Module):
    ...
    def __init__(self, point_dimension, base_potence):
        ...
        self.input_transform = TransformationNet(input_dim=point_dimension, kernel=point_dimension, output_dim=point_dimension, base_potence=base_potence)
