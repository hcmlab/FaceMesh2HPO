...
def investigate_feature_importance(full_output_dir, lightning_module: LightningModule, val_dataset,
                                   reset: bool = False):
    ...
    if not reset and os.path.exists(global_feature_importance_file):
        ...
        batch_size = 50
        test_loader = DataLoader(val_dataset, batch_size=batch_size, num_workers=1)
        for data_dict in tqdm.tqdm(test_loader, desc=f'IntegratedGradients ({fold})'):
            for key, value in data_dict.items():
                value = value.to(lightning_module.device)
                value = value.to(lightning_module.dtype)
